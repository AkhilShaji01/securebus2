...
router.get('/profile',verifyLogin,function(req, res, next) {
  ...
  //    });
    ...
    router.get('/studenttravellog',verifyLogin, function(req, res, next) {
      ...
      var clas=res1[0].classid
      console.log(res1)
      var sql="SELECT *, student.firstname, student.lastname, class.classname, studentclassmapping.classid, vehicle.regnumber, student.admissionnumber FROM dailystudent INNER JOIN student ON dailystudent.studentid = student.studentid INNER JOIN trip ON dailystudent.tripid = trip.tripid INNER JOIN vehicle ON vehicle.vehicleid = dailystudent.vehicleid INNER JOIN studentclassmapping on studentclassmapping.studentid=student.studentid WHERE studentclassmapping.classid=? and student.institutioncode = ? AND student.status = 'active';"
